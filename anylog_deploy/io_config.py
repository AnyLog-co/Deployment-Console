...
def write_configs(config_file:str=CONFIG_FILE, config_data:dict={}):
    ...
    write_msg = "Config file written to: %s" % config_file
    ...
    if not os.path.isfile(config_file):
        try:
            ...
            write_msg = 'Error: Unable to create file %s (Error: %s)' % (config_file, e)
        else:
            try:
                parser = configparser.ConfigParser()
            except Exception as e:
                write_msg = 'Error: Unbale to set parser for setting configs (Error: %s)' % e
            else:
                try:
                    ...
                    write_msg = 'Error: Failed to add content into file (Error: %s)' % e
                else:
                    try:
                        with open(config_file, 'w') as configfile:
                            try:
                                ...
                                write_msg = 'Error: Failed to write content into %s (Error: %s)' % (config_file, e)
                    except Exception as e:
                        write_msg = 'Error: Failed to open %s for content to be written (Error: %s)' % (config_file, e)
